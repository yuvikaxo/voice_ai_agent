...
def detect_intent_smart(user_input):
    """
    Use Llama 3 to classify intent from user input.
    Returns an intent string like 'set_reminder', 'open_website', etc.
    """
    prompt=f"""Classify the user's intent from the message below. Respond with one of the following intent labels only:
    - set_reminder
    - open_website
    - open_youtube
    - open_google
    ...
    chat_prompt = f"<|start_header_id|>system<|end_header_id|>\n\nYou are an intent classification assistant. You return only the intent label.\n<|eot_id|>\n"
...
def handle_intent(intent, user_input):
    """
    Executes the appropriate task based on the intent."""
    if intent == "open_website":
        # Convert input to lowercase
        user_input = user_input.lower()

        # Try to find a word that looks like a domain or website name
        words = user_input.split()
        for word in words:
            if "." in word:  # e.g., "instagram.com"
                url = f"https://{word}"
                break
            else:
                # Try guessing the site
        ...
        # Dummy parsing for now: always create reminder 1 min from now
        summary = "Default Reminder"
        reminder_time = datetime.now() + timedelta(minutes=1)
        try:
            response = create_event(summary, reminder_time)
            return f"Reminder has been set: {summary} at {reminder_time.strftime('%I:%M %p')}"
        except Exception as e:
...
def main_loop():
    ...
    while True:
        try:
            ...
            intent = detect_intent_smart(user_input)

            #Running intent task
            agent_reply=handle_intent(intent, user_input)

            # Otherwise, generate LLM response
